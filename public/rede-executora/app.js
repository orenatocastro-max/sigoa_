...
function providerDetail(nome){ const rede = rowsByPrestador(nome); const procs = uniq(rede.flatMap(r => (r.procedimentos||[]).map(p => `${p.codigo ? p.codigo+' - ' : ''}${p.nome}${p.oferta && p.oferta !== '-' ? ' • Oferta: '+p.oferta : ''}`))); const first = rede[0] || {}; return {nome, rede, municipio: uniq(rede.map(r=>r.municipio)).join(' • '), servicos: uniq(rede.map(r=>r.servico)), natureza:first.natureza, numero_contrato: uniq(rede.map(r=>r.numero_contrato)).join(' • '), contrato_fim: uniq(rede.map(r=>brDate(r.contrato_fim))).join(' • '), procedimentos: procs, bloqueios: rede.filter(r=>r.bloqueado||r.motivo_bloqueio).map(r=>({servico:r.servico, municipio:r.municipio, motivo:r.motivo_bloqueio||'Serviço bloqueado'})), observacoes: uniq(rede.map(r=>r.observacao))}; }
...
function render(){
  app.innerHTML = `<div class="proShell">
    <aside class="proSidebar">
      <div class="proBrand"><div class="brandMark">CREG</div><div><strong>Painel Rede Executora</strong><span>Coordenadoria de Regulação de Acesso</span><span>CREG/RO</span></div></div>
...
function panoramaDrawer(){ const rows=flat(); const byNature=group(rows,'natureza'); const byServ=group(rows,'servico'); const topServ=Object.entries(byServ).sort((a,b)=>b[1]-a[1]).slice(0,10); return `<div class="drawerBody"><div class="macroCard"><strong>Panorama geral</strong><span>${stats().prestadores} prestadores • ${stats().servicos} instrumentos/serviços • ${stats().municipios} municípios • ${stats().procedimentos} procedimentos vinculados</span><div class="macroNumbers"><b>${stats().bloqueados}</b><small>bloqueios</small><b>${fmt(stats().oferta)}</b><small>ofertas</small></div></div><h3 class="drawerSectionTitle">Por natureza</h3>${Object.entries(byNature).map(([n,c])=>`<button class="drawerOption" onclick='selectRows(${JSON.stringify({title:naturezaLabel(n), subtitle:'Prestadores por natureza jurídica/instrumento', rows:rows.filter(r=>r.natureza===n), sourcePanel:'panorama'})})'><strong>${esc(naturezaLabel(n))}</strong><span>${c} registro(s)</span></button>`).join('')}<h3 class="drawerSectionTitle">Serviços mais frequentes</h3>${topServ.map(([s,c])=>`<button class="drawerOption macroServiceCard" onclick='selectRows(${JSON.stringify({title:s, subtitle:'Prestadores e municípios por serviço', rows:rowsByServico(s), sourcePanel:'panorama'})})'><strong>${esc(s)}</strong><span>${c} registro(s)</span></button>`).join('')}</div>`; }
