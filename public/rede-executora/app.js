...
const valorContratoLabel = r => { const v = r?.valorGlobalContrato ?? r?.valor_global_contrato ?? ''; if(v === null || v === undefined || v === '') return 'Não informado'; const n = Number(String(v).replace(/[^0-9,.-]/g,'').replace(/\./g,'').replace(',','.')); return Number.isFinite(n) && n>0 ? n.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}) : String(v); };
...
const menu = [
  ...
  {id:'procedimentos', label:'Procedimentos', hint:'Buscar procedimento e unidades executantes'},
...
function fimVigenciaLabel(r){ return brDate(r?.contrato_fim) || 'Não informado'; }
function numeroContratoLabel(r){ return r?.numero_contrato || r?.contrato || 'Não informado'; }
function observacaoLabel(r){ return (r?.observacao || r?.observacaoGeral || '').trim() || 'Sem observação cadastrada.'; }
...
function rowResumoTexto(r){
  return `${r.prestador} • ${r.servico || '-'} • Contrato: ${numeroContratoLabel(r)} • Natureza: ${naturezaLabel(r.natureza)} • Fim da vigência: ${fimVigenciaLabel(r)} • Valor global: ${valorContratoLabel(r)} • Teto físico mensal: ${tetoLabel(r)}`;
}
...
function rowMiniHtml(row){ return `<div class="resultItem plainRecord ${row.bloqueado?'resultItemBlocked':''}"><div class="resultInfo"><div class="providerTitleBlock"><button class="providerNameButton" onclick="openProvider('${enc(row.prestador)}')">${esc(row.prestador)}</button><div class="providerMunicipioLine"><span>Tipo de serviço:</span> <strong class="municipioStrong">${esc(row.servico||'-')}</strong></div></div><div class="inlineInfoRow"><span class="inlineInfo"><b>Município</b>${esc(row.municipio||'-')}</span><span class="inlineInfo"><b>Contrato</b>${esc(numeroContratoLabel(row))}</span><span class="inlineInfo"><b>Natureza</b>${esc(naturezaLabel(row.natureza))}</span><span class="inlineInfo ${statusContrato(row).tone==='bad'?'inlineInfo-bad':statusContrato(row).tone==='critical'?'inlineInfo-critical':statusContrato(row).tone==='warn'?'inlineInfo-warn':'inlineInfo-ok'}"><b>Fim da Vigência</b>${esc(fimVigenciaLabel(row))}</span></div><p><b>Valor Global do Contrato:</b> ${esc(valorContratoLabel(row))}<br><b>Teto Físico Mensal:</b> ${esc(tetoLabel(row))}<br><b>Observação:</b> ${esc(observacaoLabel(row))}</p></div><div class="resultMeta resultMetaPlain"><span class="miniBadge">${esc(row.municipio||'')}</span><span class="miniBadge">Contrato: ${esc(numeroContratoLabel(row))}</span><span class="miniBadge">${esc(naturezaLabel(row.natureza))}</span><span class="miniBadge miniBadge-${esc(statusContrato(row).tone)}">${esc(statusContrato(row).text)}</span></div></div>`; }
