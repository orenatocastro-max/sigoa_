...
app.post('/api/prestadores/:pid/instrumentos/:iid/subescalas',roles('ADMINISTRADOR'),(req,res)=>{ if(!exigirPrestadorPermitido(req,res,req.params.pid)) return; const ps=read('prestadores.json',[]); const {p,i}=findInstrumento(ps,req.params.pid,req.params.iid); if(!i) return res.status(404).json({erro:'Instrumento não encontrado'}); const nome=String(req.body.nome||'').trim(); if(!nome) return res.status(400).json({erro:'Informe a subescala'}); i.subescalas=i.subescalas||[]; if(i.subescalas.some(s=>upperClean(s.nome)===upperClean(nome))) return res.status(400).json({erro:'Subescala já cadastrada para este serviço'}); const sub={id:uid('sub'), nome, ativo:true, observacao:req.body.observacao||''}; i.subescalas.push(sub); write('prestadores.json',ps); audit(req,'CRIAR_SUBESCALA','subescala',sub.id,[{campo:'prestador',novo:p?.nome||''},{campo:'servico',novo:i.servico||''},{campo:'subescala',novo:nome}]); res.json(sub); });
app.put('/api/prestadores/:pid/instrumentos/:iid/subescalas/:sid',roles('ADMINISTRADOR'),(req,res)=>{ const ps=read('prestadores.json',[]); const {i}=findInstrumento(ps,req.params.pid,req.params.iid); if(!i) return res.status(404).json({erro:'Instrumento não encontrado'}); i.subescalas=i.subescalas||[]; const sub=i.subescalas.find(s=>s.id===req.params.sid); if(!sub) return res.status(404).json({erro:'Subescala não encontrada'}); const old={...sub}; ['nome','ativo','observacao'].forEach(c=>{ if(req.body[c]!==undefined) sub[c]=req.body[c]; }); write('prestadores.json',ps); audit(req,'ALTERAR_SUBESCALA','subescala',sub.id,diff(old,sub,['nome','ativo','observacao'])); res.json(sub); });
  ...
  ps.forEach(p=>{
    (p.instrumentos||[]).filter(i=>i.ativo!==false).forEach(i=>{
      ...
      const subescalas=baseSubs.map(sub=>{
        ...
        const o=ofs.find(x=>x.prestadorId===p.id && x.instrumentoId===i.id && String(x.codigo||'')===String(codigo));
        const quantidade=Number(o?.quantidade||0);
        return {codigo, nome:sub.nome, quantidade, preenchida:quantidade>0, observacao:o?.observacao||'', atualizadoEm:o?.alteradoEm||o?.criadoEm||''};
      });
...
app.get('/api/contratos/alertas',auth,(req,res)=>{ const hoje=new Date(); const ps=filtrarPrestadoresPorUsuario(req, read('prestadores.json',[])); const rows=[]; ps.forEach(p=>(p.instrumentos||[]).forEach(i=>{ const fim=i.vigenciaFim; let dias=null,status='SEM_DATA'; if(fim){ const d=new Date(fim+'T12:00:00'); dias=Math.ceil((d-hoje)/86400000); status=dias<0?'VENCIDO':(dias<=30?'VENCE_30':(dias<=60?'VENCE_60':(dias<=90?'VENCE_90':'VIGENTE'))); } rows.push({prestador:p.nome,municipio:p.municipio,servico:i.servico,contrato:i.numero||'',processo:i.processo||'',vigenciaFim:fim||'',dias,status,valorGlobal:i.valorGlobal||'',prestadorId:p.id,instrumentoId:i.id}); })); rows.sort((a,b)=>(a.dias??99999)-(b.dias??99999)); res.json(rows); });
