...
function marcarEscalaLancada(req, mes, prestador){ if(!mes || !prestador) return; const escalas=read('escalas.json',[]); let e=escalas.find(x=>x.mes===mes && x.prestadorId===prestador.id); if(!e){ e={id:uid('esc'), mes, prestadorId:prestador.id}; escalas.push(e); } e.prestador=prestador.nome; e.municipio=prestador.municipio; e.status='LANÇADA'; e.operadorId=req.session.user?.id||''; e.operador=req.session.user?.nome||req.session.user?.login||''; e.login=req.session.user?.login||''; e.lancadaEm=now(); write('escalas.json',escalas); }
...
app.put('/api/usuarios/:id',roles('ADMINISTRADOR'),(req,res)=>{ const users=read('usuarios.json',[]); const u=users.find(x=>x.id===req.params.id); if(!u) return res.status(404).json({erro:'Não encontrado'}); const old={...u}; ['nome','perfil','status','motivoBloqueio'].forEach(c=>{ if(req.body[c]!==undefined) u[c]=req.body[c]; }); if(req.body.prestadoresVinculados!==undefined) u.prestadoresVinculados=Array.isArray(req.body.prestadoresVinculados)?req.body.prestadoresVinculados:[]; if(req.body.resetarSenha){ u.senha=bcrypt.hashSync(req.body.novaSenha||'123456',10); u.trocarSenha=true; }
 write('usuarios.json',users); const d=diff(old,u,['nome','perfil','status','motivoBloqueio','trocarSenha','prestadoresVinculados']); if(req.body.resetarSenha)d.push({campo:'senha',anterior:'******',novo:'senha resetada'}); audit(req,'ALTERAR_USUARIO','usuario',u.id,d); res.json({ok:true}); });

app.get('/api/prestadores',auth,(req,res)=>res.json(filtrarPrestadoresPorUsuario(req, read('prestadores.json',[]))));
app.get('/api/prestadores-todos',roles('ADMINISTRADOR','AUDITOR'),(req,res)=>res.json(read('prestadores.json',[])));
app.post('/api/prestadores',roles('ADMINISTRADOR'),(req,res)=>{ const ps=read('prestadores.json',[]); const p={id:uid('p'), nome:req.body.nome, municipio:req.body.municipio, cnpj:req.body.cnpj||'', ativo:true, observacaoGeral:req.body.observacaoGeral||'', instrumentos:[]}; ps.push(p); addAuxValue('municipios', p.municipio, {macro: macroByMunicipio(p.municipio)}); write('prestadores.json',ps); audit(req,'CRIAR_PRESTADOR','prestador',p.id,[{campo:'nome',novo:p.nome},{campo:'municipio',novo:p.municipio}]); res.json(p); });
...
app.post('/api/ofertas/lote',canWrite,(req,res)=>{ const {mes,prestadorId,instrumentoId,itens,justificativa}=req.body; if(competenciaFechada(mes) && req.session.user.perfil!=='ADMINISTRADOR') return res.status(403).json({erro:'Competência fechada'}); const ps=read('prestadores.json',[]); const {p,i}=findInstrumento(ps,prestadorId,instrumentoId); if(!i) return res.status(404).json({erro:'Instrumento não encontrado'}); if(!exigirPrestadorPermitido(req,res,prestadorId)) return; const ofs=read('ofertas.json',[]); (itens||[]).forEach(item=>{ const qtd=Number(item.quantidade||0); let o=ofs.find(x=>x.mes===mes&&x.prestadorId===prestadorId&&x.instrumentoId===instrumentoId&&onlyDigits(x.codigo)===onlyDigits(item.codigo)); const anterior=o?Number(o.quantidade||0):0; if(!o){ o={id:uid('o'),mes,prestadorId,instrumentoId,prestador:p.nome,municipio:p.municipio,servico:i.servico,codigo:item.codigo,nome:item.nome,quantidade:0,criadoPor:req.session.user.login,criadoEm:now(),alteradoPor:'',alteradoEm:'',observacao:''}; ofs.push(o); }
  ...
  }); write('ofertas.json',ofs); marcarEscalaLancada(req, mes, p); audit(req,'ESCALA_LANÇADA','prestador',prestadorId,[{campo:'prestador',novo:p.nome},{campo:'mes',novo:mes},{campo:'total_prestador_mes',novo:ofs.filter(x=>x.mes===mes&&x.prestadorId===prestadorId).reduce((sum,x)=>sum+Number(x.quantidade||0),0)}]); res.json({ok:true, totalPrestadorMes: ofs.filter(x=>x.mes===mes&&x.prestadorId===prestadorId).reduce((sum,x)=>sum+Number(x.quantidade||0),0)}); });
...
app.get('/api/escalas',auth,(req,res)=>{ const mes=String(req.query.mes||new Date().toISOString().slice(0,7)); const ps=filtrarPrestadoresPorUsuario(req, read('prestadores.json',[])); const escalas=read('escalas.json',[]).filter(e=>e.mes===mes); const ofs=read('ofertas.json',[]).filter(o=>o.mes===mes); const rows=ps.map(p=>{ const esc=escalas.find(e=>e.prestadorId===p.id); const total=ofs.filter(o=>o.prestadorId===p.id).reduce((s,o)=>s+Number(o.quantidade||0),0); const operadorNome=esc?.operador || (read('usuarios.json',[]).filter(u=>(u.prestadoresVinculados||[]).includes(p.id)).map(u=>u.nome).join(', ')); return {mes, prestadorId:p.id, prestador:p.nome, municipio:p.municipio, status: esc?'LANÇADA':'PENDENTE', operador:operadorNome||'', lancadaEm:esc?.lancadaEm||'', totalOfertaMes:total}; }); res.json(rows); });
